...
| 21 Sentence embeddings | Word embeddings, transformer encoding, similarity / yes after revised 20/19 | Represents meaning for retrieval, clustering, and semantic search | Keep after transformers; 19 → 21 → similarity search | Overload: contrastive objectives and pooling. Missing: hard negatives, anisotropy, multilingual/domain evaluation. |
| 22 LLM training pipeline | Transformers, data, optimization, distributed compute, evaluation / partial; distributed training and evaluation not yet taught | Explains pretraining, instruction tuning, preference alignment, deployment stages | Keep after transformers, but separate training lifecycle from alignment; 19 → 22A pretraining → 22B alignment | Overload: pretraining, SFT, preference learning, scaling, serving in one module. Missing: data curation/dedup, contamination, scaling laws, quantization, distributed systems basics. |
...
| 28 Reranking | Candidate retrieval, ranking metrics, cross-encoders / partial; ranking metrics are not firmly established | Improves precision on a small candidate set | Keep after retrieval evaluation basics; hybrid → retrieval metrics → reranking | Overload: bi-encoder vs cross-encoder objectives. Missing: latency budgets, training negatives, calibration. |
