...
| EVAL-02 LLM evaluation foundations | Held-out splits, token loss, adapted model stages, lexical overlap, sampling / yes through MLE-02 and NLP-03/07/08 | Decides whether a model or prompt change helped without collapsing unlike outcomes into one score | Keep between alignment and prompting; NLP-08 → EVAL-02 → NLP-04 | Rebuilt around an evaluation contract, real local model evidence, exact/F1/ROUGE-L, paired bootstrap, slices, and a retention-aware gate. Hash vectors are no longer presented as BERTScore; pass@k is an extension. |
| NLP-04 Controlled prompt engineering | Frozen-model conditioning, decoding, development/test separation, paired evaluation / yes through NLP-08 and EVAL-02 | Changes context without changing weights and decides whether the change deserves release | Keep after EVAL-02 and before guardrails; EVAL-02 → NLP-04 → NLP-05 | Rebuilt around a real local experiment with hashes, one-variable versions, development-only selection, actual generation, schema versus correctness, robustness paraphrases, uncertainty, and a rejecting gate. ReAct/tree search move to agents; deep injection defense moves to NLP-05. |
| NLP-05 LLM failure boundaries and guardrails | Controlled prompts, evaluation contracts, action confusion, trust boundaries / yes through NLP-04 and EVAL-02 | Applies an explicit policy while keeping correctness, evidence, citations, privacy, safety, and schema as separate contracts | Keep before RAG as the policy/threat foundation; NLP-04 → NLP-05 → RAG foundations. Defer real grounding to RAG-04 | Rebuilt around a measured keyword baseline and policy candidate, PII redaction, critical escapes, over-restriction, structured support/correctness cases, and an intentionally rejecting release gate. Removed simulated factuality, temperature, and RAG-reduction claims. |
