"""Builder for Notebook 25 — Similarity Search."""
import os, sys
...
cells = [
    ...
    > The embedding models from Notebooks 20–21 produce vectors in $\mathbb{R}^d$.
    > The vector databases of Notebook 26 store millions of them. Between the two sits
    ...
    **Why it matters**
    - Every RAG system (Notebooks 27–30) starts with a vector similarity search. The
    ...
    **Production databases.** As RAG pipelines matured (2022–), managed vector
    ...
    multi-tenancy, and persistence on top of ANN indexes. Notebook 26 covers these.
    """),
    ...
    **Monitoring:** track Recall@10 weekly on a golden query set (100 manually labelled
    queries); alert if Recall drops >2pp (Notebook 45). Re-index when the embedding
    model is updated (Notebook 41).
    """),
    ...
    **Next:** `26 · Vector Databases` — what managed databases add over raw FAISS:
    metadata filtering, CRUD, persistence, multi-tenancy, and replication.
    """),
]

build("phase5_rag/25_similarity_search.ipynb", cells)
