"""Builder for Notebook 20 — TF-IDF and Word Embeddings.

Run:  python3 tools/builders/phase4_20_tfidf_word_embeddings.py
Emits: notebooks/phase4_nlp_llms/20_tfidf_word_embeddings.ipynb
"""
import os
import sys
...
cells = [
    md(r"""
    # 20 · TF-IDF and Word Embeddings
    ### NLP representation foundations — taught before sequence architectures

    **Prerequisites:** Notebooks 01, 05, and 09. No RNN, attention, or Transformer
    ...
    > contextual embeddings inside the Transformer (Notebook 19); Notebook 21 shows how
    ...
    **What you will master**
    ...
    - When to use sparse vs dense retrieval (links to Notebook 25 — Similarity Search).
    ...
    **Why it matters**
    ...
    - word2vec embeddings trained in 2013 are still embedded in many production
      ...
      know when to upgrade to contextual embeddings (Notebook 21).
    ...
    **Today.** Static embeddings (one vector per word regardless of context) have been
    mostly superseded by contextual embeddings from Transformers (Notebook 19, 21), but
    ...
    **Why log?** Without it, IDF is a ratio — a term in 1 of 1000 docs would get
    ...
    code(r"""
    # 5a.5 Cosine similarity search (Notebook 01 reuse).
    def cosine_sim(a, b):
    ...
    *discriminative* component of TF-IDF: it automatically learns which words separate
    ...
    |---|---|---|---|
    | **Vocabulary mismatch** | Query term not in index → zero matches | OOV word at query or index time | BM25: expand query synonyms; FastText subword; dense retrieval (Ntbk 25) |
    ...
    | **word2vec polysemy** | "bank" has one vector for financial/river | Static embedding: one vector per word | Contextual embeddings (BERT/Ntbk 21); sense disambiguation |
    ...
    | **BM25 no semantic matching** | Query "automobile" misses docs about "car" | Keyword only, no synonym expansion | Hybrid search: BM25 + dense retrieval (Ntbk 27) |
    """),
    ...
    print("  -> Dense/hybrid retrieval (Ntbks 25, 27) solves this.")
    """),
    ...
    **Chosen architecture:** BM25 first-pass retrieval (top-100 candidates) → dense
    cross-encoder reranker (Notebook 28). This is the standard hybrid approach.
    ...
    **Constraints:** index must be rebuilt nightly; vocabulary updates as new legal
    ...
    - **Static vs contextual.** word2vec gives one vector per word regardless of context;
      ...
      upgrade to contextual embeddings (Notebook 21, BERT).
    - **Updating embeddings.** word2vec must be retrained when the vocabulary changes
      ...
      retraining for semantic drift (Notebook 45).
    ...
    **Representation type:**
    ...
    | BERT/contextual | 768 | Contextual | Yes | Slow (inference) | Disambiguation, NLU tasks (Ntbk 21) |
    ...
    **Senior (interview + production design)**
    7. *Design:* the search system for the legal document platform in §9. Include:
       ...
       (Notebook 28), latency SLAs, and monitoring (query distribution drift, Ntbk 45).
    8. *Scaling:* you have 10B tokens of in-domain legal text and a budget for 1 GPU-
       ...
       model (Ntbk 21)? Justify with expected quality, training time, and serving cost.
    ...
    **Key limitation of all methods here:** one vector per word regardless of context.
    Notebook 21 shows how to pool or contextualise these vectors into sentence-level
    representations, and Notebook 25 shows how to retrieve efficiently with them.

    **Next:** `21 · Sentence Embeddings` — BERT [CLS] pooling, mean pooling,
    sentence-transformers (bi-encoders vs cross-encoders), and how to build a semantic
    search engine from scratch.
    """),
]

build("phase4_nlp_llms/20_tfidf_word_embeddings.ipynb", cells)
