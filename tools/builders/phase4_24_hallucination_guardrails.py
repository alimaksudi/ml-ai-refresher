"""Builder for Notebook 24 — Hallucination and Guardrails."""
import os, sys
...
cells = [
    ...
    **RAG as grounding (Lewis et al., 2020; Phase 5).** Factual grounding via retrieval-
    ...
    **Figure 4.** The dramatic effect of RAG grounding on hallucination rate.
    ...
    itself being incorrect. Phase 5 (Notebooks 25–30) addresses both.
    """),
    ...
    | **Extrinsic hallucination** | Output adds facts not in source | Parametric memory overrides context | RAG (Phase 5); SelfCheckGPT detection |
    ...
    | **RAG hallucination** | Model hallucinates even with context | Relevant chunks not retrieved (Phase 5); model ignores chunks | Improve retrieval (Ntbk 25); faithfulness-trained model |
    ...
    **Mitigation stack (in layers):**
    1. **RAG grounding (Phase 5)**: retrieve from a curated, version-locked drug
    ...
    **Cost of mistakes:** FDA Class II medical device liability; clinical negligence
    ...
    - **Escape rate monitoring.** Continuously sample production outputs and run an
      independent evaluator (LLM-as-judge, Notebook 40) to estimate live hallucination
      rate. Alert if it exceeds a SLA threshold (Notebook 45).
    - **RAG as the primary mitigation.** For factual domains, RAG grounding (Phase 5)
    ...
    - **Constitutional AI / RLAIF.** At training time, fine-tune the model to output
      ...
      including them in RLHF/DPO training (Notebook 22).
    ...
    **Strong vs weak answers**
    - *"How do we reduce hallucination in our LLM-powered product?"*
      ...
      - **Strong:** "Start with RAG grounding (Phase 5) — it gives the largest
        ...
        automatic evaluator (Notebook 40) and set a SLA threshold with alerts. Fine-
    ...
    **Senior (interview + production design)**
    7. *Design:* the hallucination monitoring system for the medical Q&A product in
       §9. Include: automatic evaluator model selection (Notebook 40), sampling rate,
       alert thresholds, escalation workflow, retraining trigger conditions (Notebook
       46), and dashboarding (Notebook 45).
    ...
    validation. **Phase 4 (Modern NLP and LLMs) is now complete.**

    **Next:** `Phase 5 — Retrieval-Augmented Generation` begins with `25 ·
...
build("phase4_nlp_llms/24_hallucination_guardrails.ipynb", cells)
