...
cells.append(md(r"""# Notebook 42 — LLMOps
...
4. QUALITY SCORE  LLM judge score (NB40) or downstream task success
...
plt.savefig('/tmp/42_llmops.png', dpi=100, bbox_inches='tight')
plt.close()
print("Figure saved: /tmp/42_llmops.png")
...
| **Silent quality degradation** | Prompt + model update causes regression nobody notices | Automated LLM judge (NB40) on random sample; quality golden signal |
...
Three approaches: (1) LLM judge on random sample (NB40): route 5% of requests through a judge model →
...
build("phase8_production/42_llmops.ipynb", cells)
print("NB42 built.")
