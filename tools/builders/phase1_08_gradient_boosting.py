"""Builder for Notebook 08 — Gradient Boosting and XGBoost.

Run:  python3 tools/builders/phase1_08_gradient_boosting.py
Emits: notebooks/phase1_classical_ml/08_gradient_boosting_xgboost.ipynb
"""
import os
import sys
...
cells = [
    # ---------------------------------------------------------------- Title
    md(r"""
    # 08 · Gradient Boosting and XGBoost
    ### Phase 1 — Classical Machine Learning · *ML/AI Senior Mastery Curriculum*

    > Notebook 07 cut **variance** by *averaging* many independent deep trees.
    ...
    > **gradient descent (Notebook 03) performed in the space of functions**: each
    ...
    > fraud, credit, and ranking systems in production. This notebook closes Phase 1
    > by uniting trees (06–07) with the optimization of Phase 0.
    """),
    ...
    **Why it matters in industry**
    ...
    - Mastering the **bias-vs-variance** contrast with bagging (07) is exactly the
    ...
    **Why boosting, given we already have Random Forests?** A forest reduces variance
    but **cannot reduce the bias** shared by its trees (Notebook 07, §4.1). Boosting
    ...
    **Why shallow trees ("stumps")?** Each tree only needs to capture a *small* piece
    ...
    function-space analogue of small gradient-descent steps (Notebook 03).
    ...
    **regression tree $h_m$ to the pseudo-residuals** and take a step in that
    ...
    descent (Notebook 03) where each "step" is a tree approximating the gradient.
    ...
    - **Logistic / log loss** (binary): working in log-odds $F$ with $p=\sigma(F)$,
      ...
      saw for logistic regression (Notebook 05). Init $F_0=\log\frac{\bar y}{1-\bar y}$.
    ...
    **Figure 1.** After **1 tree** the model is a crude step (a single shallow stump's
    ...
    code(r"""
    # Figure 2 — boosting OVERFITS with too many rounds (contrast: RF plateaus, Ntbk 07).
    ...
    **Figure 2.** This is the single most important difference from Random Forest.
    ...
    Compare Notebook 07, Fig 2, where adding trees only ever *plateaus*. The practical
    ...
    **Figure 3.** A **high** learning rate (red, 0.5) drops fast but overfits early
    ...
    Notebook 03's learning-rate figure. The standard recipe: pick a **small learning
    ...
    | **Leakage amplification** | Unbelievably good CV, bad live | Boosting exploits any leaky feature hard | Rigorous validation (Notebook 10) |
    ...
    **Why gradient boosting?**
    ...
    - **Probability outputs** feed a cost-sensitive **threshold** (Notebook 05): the
    ...
    - **SHAP** (Notebook 13) gives per-transaction reason codes for analysts and
    ...
    **KPIs:** PR-AUC / recall@fixed-precision, fraud-dollars caught vs false-decline
    ...
    frequent retraining, Notebook 46).
    """),
    ...
    - **Explainability.** No coefficients; **SHAP** is the standard for per-prediction
      and global explanations (Notebook 13) and is fast for trees (TreeSHAP).
    - **Monitoring & drift.** Tabular targets like fraud/CTR drift quickly; monitor
      ...
      (Notebooks 45–46). Watch for train/serve skew in feature pipelines.
    ...
    **Phase 1 is complete.** You now command the classical-ML toolkit — linear and
    ...
    **Next:** `09 · Evaluation Metrics` — Phase 2 begins. Every model so far has been
...
build("phase1_classical_ml/08_gradient_boosting_xgboost.ipynb", cells)
