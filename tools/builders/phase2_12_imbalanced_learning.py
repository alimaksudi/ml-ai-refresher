"""Builder for Notebook 12 — Imbalanced Learning.

Run:  python3 tools/builders/phase2_12_imbalanced_learning.py
Emits: notebooks/phase2_ml_engineering/12_imbalanced_learning.ipynb
"""
import os
import sys
...
cells = [
    ...
    > model trained naively learns to ignore it (Notebooks 05, 09). This notebook is
    ...
    **What you will master**
    - *Why* imbalance breaks naive training and accuracy (recap + deepen Notebooks
      05/09), and the three families of fixes.
    ...
    **Why it matters in industry**
    - Imbalance is the *norm* in high-stakes ML; mishandling it ships a model that
      "looks accurate" and catches nothing (Notebook 09's accuracy paradox).
    - The fixes interact subtly with metrics (Notebook 09), validation (Notebook 10),
    ...
    **The naive failure.** A classifier minimizing average error (Notebooks 05, 08) on
    ...
    **Three historical lines of attack.**
    ...
    3. **Move the decision threshold (decision-level).** Keep the model and *its
       ...
       (Notebook 09). Increasingly, practitioners argue this is the **cleanest** fix:
    ...
    **The two warnings that separate seniors from juniors:**
    1. **Resample only the *training* fold.** Balancing the whole dataset before
       ...
       of test points appear in train) — fake-good CV (Notebook 10).
    ...
    flowchart TD
        ...
        I --> T["Decision-level:<br/>move threshold (Ntbk 09)"]
    ...
    The training loss is an average over examples (Notebook 08). With 99% negatives,
    ...
    ### 4.4 Threshold moving (recap of Notebook 09)
    ...
    **calibrated** model this is often the best fix — it changes the *decision*, not
    ...
    ### 4.6 The validation rule (recap of Notebook 10)
    ...
    **Figure 3.** All three "imbalance fixes" do essentially the same thing — **buy
    ...
    whether you just need a better **threshold** (Notebook 09). Often it's the latter.
    """),
    ...
    **recalibration** step on real-ratio data or a prior correction. Threshold moving
    ...
    |---|---|---|---|
    | **Resample before split** | CV ≫ live; "perfect" minority recall | Synthetic/dup minority points leak across folds | Resample **inside** the fold (imblearn Pipeline, Ntbk 10) |
    ...
    | **Wrong metric** | "Improved accuracy," worse business outcome | Accuracy on imbalanced data | PR-AUC, recall@precision, cost (Ntbk 09) |
    ...
    **Why this is the textbook imbalance problem:**
    - **Accuracy is meaningless** — "all healthy" scores 99.5% (Notebook 09).
    ...
    **Approach (senior playbook):**
    ...
    2. **Move the threshold** to the cost-optimal point from the FN/FP matrix
      (Notebook 09) — usually the biggest lever.
    ...
    **Constraints:** clinical capacity caps the false-positive volume; outputs must be
    auditable and calibrated; rigorous, leak-free validation (Notebook 10).
    ...
    **KPIs:** recall at fixed FP-rate (sensitivity at fixed specificity), PR-AUC,
    ...
    - **Resample train-fold-only, always.** Use an imblearn `Pipeline`; never resample
      ...
      correctness rule (§7, Notebook 10).
    - **Mind calibration.** If the probability is consumed downstream, prefer class
      ...
      resampled model on real-ratio data (Notebook 09).
    ...
    - **Metric discipline.** Track PR-AUC / recall@precision / cost, never bare
      accuracy (Notebook 09). Report with confidence intervals (few positives → noisy
      metrics; bootstrap, Notebook 02).
    ...
    **Deep-dive questions**
    ...
    - *Why resample inside CV?* → Resampling before the split leaks minority info
      across folds → fake-good CV (Fig in §7, Notebook 10).
    ...
    **decision-level** (**threshold moving**), and they mostly do the same thing: trade
    ...
    Notebook 10), **recalibrate** after resampling, and judge with **PR-based metrics**
    (Notebook 09), not accuracy.

    **Next:** `13 · Explainability (SHAP)` — our tree ensembles (07–08) and the models
    here are accurate but opaque. We close Phase 2 by making any model's predictions
    *explainable* with a principled, game-theoretic method, essential for trust,
    debugging, and regulated decisions.
    """),
]

build("phase2_ml_engineering/12_imbalanced_learning.ipynb", cells)
