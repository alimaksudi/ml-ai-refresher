"""Builder for Notebook 11 — Feature Engineering.

Run:  python3 tools/builders/phase2_11_feature_engineering.py
Emits: notebooks/phase2_ml_engineering/11_feature_engineering.ipynb
"""
import os
import sys
...
cells = [
    ...
    > (Notebook 10). The single most dangerous technique here, **target encoding**,
    ...
    **What you will master**
    - **Numeric transforms**: standardization, min-max, **log / power** for skew,
      and **binning** — and which models need them (Notebooks 03–05) vs which don't
      (trees, Notebooks 06–08).
    - **Categorical encoding**: one-hot, ordinal, and **target/mean encoding** done
      **leak-free** via out-of-fold cross-fitting + smoothing (the Notebook-10 link).
    ...
    **Models are simple; the world is messy.** The algorithms of Phase 1 expect clean
    ...
    **Why representation matters so much.** A linear model can only fit a line; give it
    ...
    idea as a kernel or a basis expansion (Notebook 04's polynomial features): we do
    ...
    **The deep-learning twist — and why this still matters.** Deep nets *learn*
    ...
    engineering faded for images and text (Phases 3–4). But for **tabular data** —
    ...
    Moreover, the *discipline* transfers: embeddings (Notebooks 20–21) are learned
    features, and prompt design (Notebook 23) is feature engineering for LLMs.

    **The cautionary half.** Feature engineering is also the **#1 source of data
    leakage** (Notebook 10): target encoding, aggregates, and "helpful" derived columns
    ...
    **Figure 1.** Same data, same linear model — only the input representation changed.
    ...
    - **Standardization** $z=\frac{x-\mu}{\sigma}$ — zero mean, unit variance. Required
      for distance/gradient/regularized models (Notebooks 03–05); irrelevant to trees.
    ...
    **Fit statistics on TRAIN only.** $\mu,\sigma,\min,\max$, quantile edges must come
    ...
    preprocessing leakage (Notebook 10).
    ...
    folds only (and at serving, from the full training set). This is the Notebook-10
    ...
    model can fit curves and interactions — the same trick as Notebook 04's polynomial
    ...
    aggregates** (point-in-time correct! Notebook 10). Cyclical features (hour, month,
    ...
    **impute + add a binary "was-missing" flag** so the model can learn that
    ...
    - **Embedded**: selection inside training — **L1/Lasso** zeros coefficients
      (Notebook 04), tree **importances** (Notebook 07). Usually the best cost/quality
      trade.
    """),
    ...
    code(r"""
    # 5.2 Smoothed target encoding with OUT-OF-FOLD cross-fitting (leak-free, Ntbk 10).
    ...
    **Figure 3.** Target encoding is powerful for high-cardinality categoricals — but
    ...
    canonical feature-engineering leak from Notebook 10 — *always* cross-fit any
    ...
    *right* representation — matches or beats the forest. The lesson senior engineers
    ...
    | **Preprocessing leakage** | Optimistic CV | Scaler/encoder fit on all data | Fit inside `Pipeline`/fold (Ntbk 10) |
    ...
    | **Train/serve skew** | Offline ≠ online | Features computed differently live | Shared pipeline; feature store (Ntbk 43) |
    ...
    print("For very high cardinality, also consider feature hashing or learned embeddings (Ntbk 20-21).")
    """),
    ...
    **The feature work is the project:**
    ...
    - **Lags & rolling aggregates:** demand in the previous windows, last week
      same-hour — **point-in-time correct** (Notebook 10/43) or you leak the future.
    ...
    **KPIs:** forecast error (MAE/RMSE per zone-window, Notebook 09), realized
    ...
    - **Point-in-time correctness.** Lags and aggregates must use only data available at
      prediction time; this is the core job of a **feature store** (Notebook 43) and a
      frequent leakage source (Notebook 10).
    ...
    - **Feature drift & freshness.** Monitor feature distributions (Notebook 45); stale
    ...
    **Categorical encoding:**
    ...
    | Learned embedding | Low-dim dense | **Well** | Low | Deep models, huge cardinality (Ntbk 20) |
    ...
    **leak-free** (Notebook 10) and **point-in-time correct** at serving.

    **Next:** `12 · Imbalanced Learning` — we return to the rare-positive problem that
    haunted Notebooks 05/09 and tackle it head-on: resampling, class weights,
    threshold-moving, and the metrics that keep you honest when one class is 1% of the
    data.
    """),
]

build("phase2_ml_engineering/11_feature_engineering.ipynb", cells)
