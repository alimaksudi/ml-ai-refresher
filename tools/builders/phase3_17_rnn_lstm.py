"""Builder for Notebook 17 — RNNs and LSTMs.

Run:  python3 tools/builders/phase3_17_rnn_lstm.py
Emits: notebooks/phase3_deep_learning/17_rnn_and_lstm.ipynb
"""
import os
import sys
...
cells = [
    # ---------------------------------------------------------------- Title
    md(r"""
    # 17 · Recurrent Neural Networks and LSTMs
    ### Phase 3 — Deep Learning Foundations · *ML/AI Senior Mastery Curriculum*

    **Prerequisites:** Notebooks 15A and 20. You should understand stable neural
    training, tokens, vocabularies, sparse text baselines, and word embeddings.

    > CNNs (Notebook 16) exploited *spatial* structure by sharing weights across
    ...
    > runs straight into the **vanishing-gradient** problem from Notebook 15, now
    ...
    > are the conceptual bridge to **attention** (Notebook 18) and **Transformers**
    > (Notebook 19), which replaced them by removing the sequential bottleneck.
    """),
    ...
    **What you will master**
    ...
    - **Backpropagation through time (BPTT)** and why it causes **vanishing/exploding
      gradients** over long sequences (the Notebook-15 problem, in time).
    ...
    **Why feed-forward nets can't handle sequences.** An MLP (Notebook 14) expects a
    fixed-size input and treats positions independently; a CNN (Notebook 16) captures
    ...
    **vanishes** (→ early steps are forgotten) or **explodes** (→ NaNs) — the
    Notebook-15 phenomenon, amplified by depth-in-time.
    ...
    **The Transformer disruption (2017).** RNNs/LSTMs are inherently **sequential** —
    ...
    struggle with very long contexts. **Attention** (Notebook 18) lets every position
    directly access every other in parallel, and **Transformers** (Notebook 19) built
    ...
    **The LSTM's gated memory (the key idea).** Add a **cell state** $c_t$ that acts
    ...
    backprop (Notebook 15). The gradient w.r.t. $h_t$ flows backward through the
    recurrence:
    ...
    this is far more severe than ordinary depth (Notebook 15). Exploding gradients are
    patched by **gradient clipping** (Notebook 03); vanishing needs an architectural
    fix — the LSTM.
    ...
    **undiminished** across many steps — no vanishing. The gates *learn* what to keep,
    ...
    attention** (Notebook 18) removes both limits: every position attends to every other
    ...
    *constant error carousel*. The gates *learn* when to hold vs erase, giving
    ...
    | **Exploding gradients** | Loss → NaN; spikes | Repeated multiplication >1 | **Gradient clipping** (Ntbk 03); careful init |
    | **No parallelism** | Slow training on long sequences | Inherently sequential ($h_t$ needs $h_{t-1}$) | Truncated BPTT; or switch to Transformers (Ntbk 19) |
    ...
    **Scenario.** A utility forecasts **hourly electricity demand** from historical
    ...
    cyclical features of Notebook 11), and dependence on recent hours.
    ...
    **Why an LSTM (and when not):**
    ...
    - *But*: for tabular-ized time series, **gradient boosting** (Notebook 08) on
      lag/rolling features is a very strong, simpler baseline; and for long contexts,
      **Transformers** (Notebook 19) often win. A senior baselines all three.
    ...
    **Constraints:** strict **point-in-time correctness** and time-ordered validation
    (Notebook 10 — never shuffle!); real-time inference; robustness to regime shifts
    (heat waves, holidays).

    **KPIs:** MAE/RMSE/MAPE per horizon (Notebook 09), peak-hour error, forecast
    interval coverage, and the offline↔online gap under distribution shift
    (Notebook 45 → retrain, 46).
    """),
    ...
    - **Gradient clipping is standard** for any recurrent model — cheap insurance
      against exploding gradients NaN-ing a run (§7, Notebook 03).
    - **Time-ordered validation only.** Never shuffle a sequence/time series across the
      split; use forward-chaining and point-in-time features (Notebook 10). This is the
    ...
    - **Sequential = hard to parallelize.** Training is slower than CNNs/Transformers on
      ...
      Transformer is a better fit (Notebook 19).
    ...
    **Recurrent nets vs Transformers (the modern contrast — Ntbk 18–19):**
    ...
    **Common mistakes:** shuffling time series in validation (Notebook 10); forgetting
    ...
    **BPTT** runs into **vanishing/exploding gradients** over long ranges — the
    Notebook-15 problem stretched across time — so vanilla RNNs forget the distant past.
    ...
    **Next:** `18 · The Attention Mechanism` — the idea that lets every position look
    ...
    Transformer (Notebook 19) and all modern LLMs.
    """),
]

build("phase3_deep_learning/17_rnn_and_lstm.ipynb", cells)
