"""Builder for Notebook 10 — Validation and Data Leakage.

Run:  python3 tools/builders/phase2_10_validation_leakage.py
Emits: notebooks/phase2_ml_engineering/10_validation_and_data_leakage.ipynb
"""
import os
import sys
...
cells = [
    # ---------------------------------------------------------------- Title
    md(r"""
    # 10 · Validation and Data Leakage
    ### Core ML Evidence — required before trees, ensembles, and tuning

    **Prerequisites:** Notebooks 03A, 04–05, and 09. You should understand the
    prediction unit, a holdout split, model fitting, and task-aligned metrics.

    > Notebook 03A introduced the first holdout and Notebook 09 gave us task-aligned
    ...
    **Why it matters in industry**
    ...
    - Correct validation is the foundation under *every* metric, model comparison,
      and A/B decision; if it's wrong, all of Phase 1–2 is wrong.
    - **Point-in-time correctness** and train/serve skew are core to feature stores
      and MLOps (Notebooks 43, 45).
    ...
    **The original sin: evaluating on training data.** The first lesson of ML is that
    ...
    memorizes its training set perfectly (Notebook 06). So the **holdout** test set
    ...
    *as long as the held-out data leaks nothing*.
    ...
    class proportions per fold (essential for imbalanced data, Notebook 09).
    **LOOCV** ($k=n$) is nearly unbiased but high-variance and expensive.

    ### 4.4 When plain K-fold is *wrong*
    Random K-fold assumes rows are **IID** (Notebook 02). When they're not, shuffling
    leaks:
    ...
    *next* segment, mimicking how the model will actually be used (predict the future
    ...
    validate this way, and features must be **point-in-time correct** (Notebook 43).
    """),
    ...
    *which* problem you have — and it's only meaningful when the validation split is
    ...
    | **Train/serve skew** | Offline ≠ online despite clean CV | Features computed differently at serving | Shared feature pipeline; monitor (Ntbk 45) |
    | **High-variance CV** | Scores swing across folds | Too few samples / unstable model | More folds/repeats; report CI (Ntbk 02) |
    ...
    **The fix and KPIs:** rebuild with `GroupKFold`, a feature audit removing
    ...
    intervention (a proper experiment, Notebook 02).
    """),
    ...
    - **Point-in-time correctness.** For any time-ordered problem, every feature must
      ...
      a **feature store** (Notebook 43) and the hardest part of real ML systems.
    ...
    - **Offline↔online reconciliation.** Always confirm offline metrics with an online
      A/B test on the real KPI (Notebook 02); a large gap is a leakage/skew alarm
      (Notebooks 45–46).
    ...
    **Next:** `11 · Feature Engineering` — with trustworthy evaluation in hand, we turn
...
build("phase2_ml_engineering/10_validation_and_data_leakage.ipynb", cells)
