"""Builder for Lesson RAG-05 — Vector Databases."""
import os, sys
...
cells = [
    md(r"""
    # RAG-05 · Vector Databases
    ### Section 06 — Retrieval-Augmented Generation · *ML/AI Senior Mastery Curriculum*

    > **Prerequisites:** RAG-01 through RAG-04. RAG-01 taught similarity search
    > algorithms (LSH, HNSW, IVF+PQ) and FAISS. RAG-03 and RAG-04 established a
    > labelled retrieval and grounded-answer baseline that this lesson must preserve.
    ...
    **What you will master**
    ...
    - **Measured migration**: persist the existing curriculum corpus in Qdrant local
      mode and prove retrieval quality, citation provenance, and policy behavior do
      not regress after restart.

    **Why it matters**
    - Every production RAG system (RAG-01 through RAG-08) requires a vector database.
    ...
    **2023–: Horizontal scaling.** As RAG pipelines became standard (LangChain 2023,
    ...
    quantised storage (Lesson RAG-06 hybrid search).
    ...
    **Figure 3.** Post-filter recall at fixed over-fetch=$20\times$ collapses as
    ...
    md(r"""
    ## 8 · Production Library Implementation and Measured Migration
    """),
    ...
    try:
        ...
        lines_qdrant = [
            ...
            '  results = client.query_points("col", query=q, limit=10)',
        ]
        print('\n'.join(lines_qdrant))
    """),

    md(r"""
    ### 8.2 · Executable persistent Qdrant baseline

    The printed API sketch above becomes useful only when it is connected to labelled
    evaluation. The project below uses the same sentence chunks, deterministic LSA
    vectors, and 18 gold queries from Lesson RAG-03. NumPy exact search is the control;
    Qdrant local exact search is the storage migration. We require matching rankings,
    then close and reopen the database and repeat a query.

    Install once with `pip install -r projects/rag_foundations/requirements.txt`.
    Local mode exercises collections, point IDs, payloads, filters, upserts, deletion,
    and disk persistence without a server. With only 70 points, it does **not** measure
    production HNSW capacity or distributed latency.
    """),
    ...
    from rag_foundations.vector_store import evaluate_vector_store

    with tempfile.TemporaryDirectory(prefix='rag05-vector-store-') as temporary_dir:
        report = evaluate_vector_store(
            data_dir=ROOT / 'projects/rag_foundations/data',
            index_dir=Path(temporary_dir) / 'index',
        )

    print('NumPy exact recall@5:', report['numpy_exact']['recall_at_k'])
    print('Qdrant local recall@5:', report['qdrant_local_exact']['recall_at_k'])
    print('Ranking parity:', report['parity'])
    print('Persistence checks:', report['persistence'])
    print('Policy filters:', report['policy_filters']['checks'])
    """),

    md(r"""
    **Expected result:** both stores should retain at least 0.90 recall@5 and have
    1.00 section-ranking parity. The point count must remain unchanged after a second
    upsert, the first query must return the same records after restart, and every
    policy check must be `True`. If retrieval quality changes, investigate vector
    normalization, distance metric, score threshold, IDs, and filters before tuning ANN.
    """),
    ...
    **Why not Pinecone?** At 500 tenants × 100K docs each = 50M total vectors, Pinecone's
    ...
    - **Cost levers.** PQ compression (Lesson RAG-01) reduces memory 16–128× at cost of
    ...
    **System design question**
    - "Design the vector storage layer for a legal document search system: 5M contracts,
      ...
      HNSW index per collection; sparse+dense hybrid (Lesson RAG-06) for keyword support;
    ...
    **Senior (design)**
    ...
    ### Mini-project — Storage migration without retrieval regression

    - **Goal:** move the measured curriculum assistant from NumPy arrays to persistent
      Qdrant local storage without changing retrieval behavior.
    - **Dataset fields:** chunk ID, document ID, section ID, heading, strategy,
      access level, freshness, unsafe-content flag, and dense vector.
    - **Workflow:** fit the declared encoder, establish exact results, upsert stable
      IDs and payloads, rerun queries, restart the store, test filters, and compare.
    - **Expected output:** `vector_store_evaluation.json` containing retrieval metrics,
      row-level evidence, ranking parity, persistence, idempotency, and policy checks.
    - **Evaluation:** recall@5 >= 0.90, ranking parity = 1.00, restart match = true,
      duplicate upsert idempotent = true, and every policy check = true.
    """),
    ...
    **Related lesson:** `RAG-06 · Hybrid Search` — combining dense vector search with sparse BM25/TF-IDF
...
build("06_rag/05_vector_databases.ipynb", cells)
