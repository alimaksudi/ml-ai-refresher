"""Builder for Notebook 19 — Transformers.

Run:  python3 tools/builders/phase3_19_transformers.py
Emits: notebooks/phase3_deep_learning/19_transformers.ipynb
"""
import os
import sys
...
cells = [
    ...
    > Notebook 18 derived the **attention mechanism**. The Transformer (Vaswani et al.,
    ...
    > level task, and close Phase 3 by connecting every component back to what we built
    > in Notebooks 14–18.
    """),
    ...
    **What you will master**
    ...
    - **Positional encoding**: why attention needs it (Notebook 18's permutation-
    ...
    - **Scaling laws**, **pre-training vs fine-tuning**, and the production landscape
      of LLMs that Phase 4 builds on.

    **Why it matters**
    - Every topic in Phases 4–6 (LLMs, RAG, Agents) assumes fluent understanding of
      the Transformer architecture. Phase 4 starts with how Transformers are trained
      on text (Notebook 22) and extended with prompting and fine-tuning (Notebooks
      23–24).
    ...
    **Before the Transformer.** NLP in 2017 combined LSTMs (Notebook 17) with attention
    (Notebook 18), using recurrence to process sequences and attention to let the
    ...
    **The Transformer block as a two-stage filter.** Each block does two things:
    ...
    the input — like ResNets (Notebook 15's vanishing-gradient fix) applied to NLP.
    ...
    **Layer norm stabilises the residual stream.** Without normalisation the residual
    ...
    conditioned (analogous to standardisation in Notebooks 03/11).

    **Positional encoding is the "address label."** Attention is permutation-invariant
    (Notebook 18, §7); to give positions meaning, we add a positional signal to each
    ...
    **Stacking blocks = hierarchical representation.** Early layers learn local,
    ...
    as CNNs (Notebook 16), but in semantic space.
    ...
    **RoPE** (Rotary Position Embedding) which bakes relative position into the Q/K
    ...
    For a sequence $x_1,\dots,x_T$, the cross-entropy loss (Notebook 02) is
    $$J = -\frac{1}{T-1}\sum_{t=1}^{T-1}\log p(x_{t+1}\mid x_1,\dots,x_t).$$
    With causal masking (Notebook 18), all $T-1$ next-token predictions are computed
    ...
    print("backprop (Ntbk 15) + Adam (Ntbk 03) would drive this to near-zero for this easy task.")
    """),
    ...
    **Figure 3.** Without positional encoding the attention pattern depends only on
    ...
    otherwise *completely* position-agnostic (Notebook 18 §7).
    """),
    ...
    | **Hallucination** | Plausible but false generation | LM objective only maximises token prob | RLHF/DPO alignment; RAG grounding (Ntbk 24) |
    ...
    **Figure 4.** Without layer normalisation the residual stream's variance grows
    exponentially with depth (the same vanishing/exploding problem as Notebook 15,
    ...
    **Scratch vs production.** Our NumPy mini-GPT implements the exact same forward
    ...
    handles beam search, top-p, temperature, stopping criteria). Phase 4 builds on this
    foundation.
    """),
    ...
    **Constraints:** context window (must fit conversation + retrieved docs — Notebook
    ...
    hallucination and drift (Notebooks 24, 45).

    **KPIs:** answer accuracy on a golden QA set, hallucination rate (Notebook 24),
    ...
    - **Flash Attention** (IO-aware exact attention, Notebook 18) is the default for
    ...
    - **Alignment** (RLHF/DPO): a language model trained on next-token prediction is
      ...
      (Notebook 24).
    - **Context window management** (Notebook 29): documents that don't fit the context
      must be chunked and retrieved (RAG, Phase 5), not truncated naively.
    - **Monitoring**: track output distribution, hallucination rate, and latency; set
      up RLHF/human-feedback loops for continuous improvement (Notebooks 39–40).
    ...
    **Fine-tuning strategy:**
    ...
    | Prompt engineering | Zero | None | Quick experiments (Ntbk 23) |
    | RAG | Low (no gradient) | Freshness | Factual/knowledge-heavy tasks (Phase 5) |
    ...
    **Common questions**
    ...
    - *Why layer norm and residual connections?* → Residual: each block learns a
      correction, not a full transformation (Notebook 15); layer norm: stabilises scale
      (Fig 4, Notebook 03/11).
    ...
    **Deep-dive questions**
    ...
    - *Flash Attention?* → IO-aware tiling of the $T^2$ attention computation to SRAM;
      exact, $O(T)$ memory, ~2–4× faster (Notebook 18 §10).
    ...
    **Strong vs weak answers**
    ...
    - *"Our Transformer loses coherence on sequences >4K tokens."*
      ...
      - **Strong:** "The model's PE and attention patterns may not extrapolate beyond
        ...
        RAG (Phase 5) is cleaner than extending context blindly."
    ...
    **Common mistakes:** not knowing pre-norm vs post-norm; thinking attention is
    ...
    2. **Why was it invented?** What RNN/LSTM limits does it fix (Notebook 17)?
    ...
    **Senior (interview + production design)**
    ...
    8. *Design:* the fine-tuning pipeline for the customer support LLM of §9 — model
       selection, LoRA vs full fine-tuning, data curation, evaluation (Notebook 38),
       alignment (Notebook 24), and drift monitoring (Notebook 45).
    ...
    **positional encoding** to give attention its missing sense of order (Fig 1). This
    ...
    to its motivation: residuals from Notebook 15, attention from Notebook 18, LN from
    Notebook 03, loss from Notebook 02.

    **Phase 3 (Deep Learning) is now complete.** You can build the full deep-learning
    stack from first principles: MLP → backprop → CNN → RNN/LSTM → attention →
    Transformer.

    **Next:** `Phase 4 — NLP and LLMs` begins with `20 · TF-IDF and Word Embeddings` —
...
build("phase3_deep_learning/19_transformers.ipynb", cells)
