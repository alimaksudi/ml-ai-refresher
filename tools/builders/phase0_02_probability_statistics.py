"""Builder for Notebook 02 — Probability and Statistics.

Run:  python3 tools/builders/phase0_02_probability_statistics.py
Emits: notebooks/phase0_foundations/02_probability_and_statistics.ipynb
...
cells = [
    ...
    **Prerequisites:** Notebooks 00A–00D and Notebook 01. You should be able to
    read fractions, summations, logarithms, subscripts, and basic derivatives.
    **Estimated time:** 4–6 hours including exercises.

    > Notebook 01 gave us the *geometry* of data (vectors, matrices, SVD). This
    ...
    **Notation bridge.** $P(A)$ means the probability of event $A$;
    ...
    00C introduces each symbol with small examples.
    ...
    **Covariance** measures co-movement: $\operatorname{Cov}(X,Y)=\mathbb
    ...
    cousin of $X^\top X$ from Notebook 01 (and PCA diagonalizes it).
    ...
    (time series, grouped data) silently breaks evaluation (Notebook 10).
    ...
    **Symbols:** $\mu$ (mu) is the Gaussian center; $\sigma^2$ (sigma squared) is
    ...
    > (Notebook 05). Choosing a loss = choosing a noise model.
    ...
    **Figure 4.** We never derived a formula for the standard error of a *median* —
    ...
    | **Non-IID data** | Great offline, bad online | Temporal/grouped leakage breaks independence | Time-based & grouped splits (Notebook 10) |
    ...
    **KPIs:** primary = conversion rate (with CI); guardrails = revenue/user, page
    ...
    - **Drift = distribution change.** Monitor feature/label distributions over
      ...
      is data drift; a shift in $P(y\mid x)$ is concept drift (Notebook 45).
    ...
    **Next:** `03 · Optimization and Gradient Descent` — having defined *what* to
...
build("phase0_foundations/02_probability_and_statistics.ipynb", cells)
