"""Builder for Notebook 16 — Convolutional Neural Networks (CNN).

Run:  python3 tools/builders/phase3_16_cnn.py
Emits: notebooks/phase3_deep_learning/16_cnn.ipynb
"""
import os
import sys
...
cells = [
    ...
    **Prerequisites:** Notebooks 13A–15A. You should be able to train and evaluate
    ...
    > The MLP of Notebook 14 treats every input as an unrelated number — fine for
    ...
    > trains a real CNN with the backprop of Notebook 15.
    """),
    ...
    **What you will master**
    - *Why* dense MLPs fail on images: **parameter explosion**, **no translation
      invariance**, and **ignoring spatial locality** (the Notebook-14 contrast).
    ...
    **Why it matters in industry**
    ...
    - The *inductive biases* (locality, weight sharing) are the template for
      ...
      positions" idea reappears in Transformers (Notebook 19).
    ...
    **The dense-net wall for images.** Notebook 14's MLP connects every input to every
    ...
    **The 2012 breakthrough.** **AlexNet** (Krizhevsky, Sutskever, Hinton, 2012) won
    ...
    Successors (VGG, GoogLeNet, **ResNet** with residual connections — Notebook 15's
    ...
    **Stacking builds a hierarchy.** Early layers detect **edges**; combining edges
    ...
    representation learning" (Notebook 14, Fig 5) specialized for space.
    ...
    *prior* that drastically cuts parameters and lets a feature detected in one place
    ...
    After convolution, apply a nonlinearity (ReLU, Notebook 14) elementwise, then
    optionally **pool**:
    ...
    ### 4.6 Backprop through conv & pool (Notebook 15)
    ...
    **feature map** highlighting where that pattern occurs. The Sobel filters fire on
    ...
    Notebook 15) discovers whatever filters minimize the loss, and the first layer
    ...
    **Figure 4.** Starting from random noise, gradient descent (§5.2) shaped the 3×3
    ...
    program the filters, you *learn* them end-to-end with backprop (Notebook 15). In a
    ...
    | **Vanishing gradients (deep CNN)** | Early layers don't learn | Multiplied Jacobians (Ntbk 15) | ReLU, BatchNorm, **residual connections** (ResNet) |
    ...
    | **Distribution shift** | Great offline, bad on new cameras/lighting | Train/serve image distribution differs | Domain adaptation; monitor (Ntbk 45) |
    ...
    (im2col + GEMM, or Winograd/FFT) GPU kernels, with autograd (Notebook 15) handling
    ...
    **Scratch vs production.** Our triple-nested loop in `conv2d` is correct but slow;
    ...
    Notebook-15 vanishing-gradient fix), data augmentation, and pretrained backbones.
    """),
    ...
    **Cost of mistakes (asymmetric, ties to Notebooks 09/12):**
    ...
    the threshold from the cost matrix (Notebooks 09, 12), and report PR-AUC.
    ...
    **Constraints:** real-time latency on edge hardware; robustness to lighting/camera
    ...
    saw the defect, Notebook 13).

    **KPIs:** recall@precision, PR-AUC, per-frame latency, throughput (units/sec), and
    drift in image statistics as cameras/lighting change (Notebook 45 → retrain,
    Notebook 46).
    """),
    ...
    - **Robustness & monitoring.** Vision models are sensitive to distribution shift
      ...
      and performance, and retrain on shifted data (Notebooks 45–46).
    - **Explainability.** Use Grad-CAM/saliency or SHAP (Notebook 13) to verify the model
    ...
    - **Memory.** Activations (not just weights) dominate memory for images; use
      checkpointing (Notebook 15) and appropriate batch sizes.
    """),
    ...
    **CNN vs Vision Transformer (ViT) — the modern contrast (preview of Ntbk 19):**
    ...
    **Intermediate (analysis)**
    ...
    6. Implement **backprop through a conv layer** (§4.6) and gradient-check it
       (Notebook 15) against finite differences.
    ...
    **Senior (interview + production design)**
    ...
    objects — all via the backprop of Notebook 15. The filters are *learned* feature
    ...
    **Next:** `17 · RNNs and LSTMs` — we turn from spatial data to **sequential** data
    ...
    to combat the vanishing gradients we met in Notebook 15.
    """),
]

build("phase3_deep_learning/16_cnn.ipynb", cells)
