"""Builder for Notebook 14 — Neural Networks from Scratch.

Run:  python3 tools/builders/phase3_14_neural_networks_scratch.py
Emits: notebooks/phase3_deep_learning/14_neural_networks_from_scratch.ipynb
"""
import os
import sys
...
cells = [
    ...
    **Prerequisites:** Notebooks 03, 05, and 13A. You should be able to explain
    ...
    > Phase 1's models drew straight boundaries (linear/logistic) or axis-aligned boxes
    ...
    > (Notebook 05), with a **nonlinearity** between them — and that single addition
    ...
    > forward pass as matrix multiplies (Notebook 01), the cross-entropy loss
    > (Notebook 02), and a manual training loop (Notebook 03). We derive the gradients
    > for our specific 2-layer net by hand here; Notebook 15 generalizes that into the
    ...
    **What you will master**
    - The neuron as a **logistic-regression unit**, and a network as **stacked layers**
      of them with nonlinear activations between (the Notebook-05 link).
    ...
    **Why it matters in industry**
    ...
    - "Deep learning = representation learning" is the paradigm shift from hand-crafted
      features (Notebook 11) to learned ones.
    ...
    **The perceptron (Rosenblatt, 1958).** The first trainable neuron: a weighted sum
    passed through a step function — essentially Notebook 05's logistic unit with a
    ...
    **The XOR winter (Minsky & Papert, 1969).** Minsky and Papert proved a single
    ...
    linear unit can only draw a line, Notebook 05) helped trigger the first "AI winter":
    ...
    *engineer* nonlinearity (Notebook 11); trees carve axis-aligned boxes (Notebook 06).
    A neural network **learns a smooth, nonlinear representation directly from data** —
    no feature engineering, no axis alignment. The price (which the rest of Phase 3
    ...
    **A neuron is a logistic regression.** Recall Notebook 05: compute a weighted sum
    ...
    Notebook 01). A **network** stacks layers: each layer's outputs feed the next.
    ...
    **Universal approximation, intuitively.** Each hidden unit contributes a soft
    ...
    flowchart LR
        ...
        O --> L["loss (cross-entropy, Ntbk 02)"]
        L -.->|"gradients (Ntbk 15)"| H1
    ```
    ...
    where $g$ is applied elementwise. The matrix multiply (Notebook 01) computes all
    ...
    *exactly* logistic regression (Notebook 05) applied to the **learned features**
    ...
    NLL). Same loss-design logic as Phase 1 — choosing the loss = choosing the noise
    model.
    ...
    (as in Notebook 05): $\;dZ_2=\hat y - y$. Then propagate backward through the layer:
    ...
    reusing each layer's stored activations. **Notebook 15 turns this hand-derivation
    ...
    *A feed-forward network with a single hidden layer containing finitely many neurons
    ...
    from §4.6, and a gradient-descent training loop (Notebook 03). We first prove the
    *motivation* — a linear model cannot learn XOR — then watch the MLP succeed.
    ...
    class MLP:
        def __init__(self, n_in, n_hidden, seed=0, activation="tanh"):
            ...
            # Xavier/He-style init: scale by fan-in to keep activations well-scaled (Ntbk 03 conditioning)
    ...
    **Figure 2.** The activation injects the nonlinearity that prevents layer collapse
    (§4.3). Their **derivatives** matter for training (Notebook 15): sigmoid and tanh
    **saturate** — their gradient $\to 0$ for large $|z|$, so a saturated neuron stops
    ...
    Xm = (Xm - Xm.mean(0)) / Xm.std(0)               # standardize (Ntbk 03/11)
    ...
    md(r"""
    **Figure 3.** The loss falls smoothly as gradient descent (Notebook 03) updates the
    ...
    gradient-based model (Notebooks 03–05), NNs train far better on well-scaled data.
    """),
    ...
    **Figure 4.** With only **2 hidden units** the network can bend space a little —
    ...
    of Notebook 04 returns, now controlled by width/depth and regularization). This is
    ...
    **Figure 5.** This is the most important figure in the notebook. In the **input
    ...
    it's the principle behind CNNs learning edges→shapes (Notebook 16) and Transformers
    learning contextual meaning (Notebook 19).
    """),
    ...
    | **Vanishing gradients** | Deep net's early layers don't learn | Sigmoid/tanh saturate, $g'\to0$ (Fig 2) | ReLU; normalization; residuals (Ntbk 16) |
    | **Dead ReLUs** | Many units stuck at 0 output | Large negative pre-activations → zero gradient | Lower LR; LeakyReLU; better init |
    | **Unscaled inputs** | Slow/unstable training | Ill-conditioned loss (Ntbk 03) | Standardize features |
    | **Bad learning rate** | Diverges or crawls | Step too big/small (Ntbk 03) | Tune LR; schedules; Adam |
    | **Overfitting** | Train ≫ test | Too many params for the data | Regularization, dropout, more data, early stop |
    | **Non-convexity** | Different runs, different solutions | Loss surface has many minima/saddles (Ntbk 03) | Good init, SGD noise, multiple runs |
    ...
    computation graph and computes them for you (the general backprop of Notebook 15),
    ...
    **Scratch vs production.** Our NumPy MLP and the PyTorch version implement the same
    ...
    again), plus GPU acceleration, optimizers (Adam, Notebook 03), layers, batching,
    ...
    misbehaves. Notebook 15 derives the general autograd/backprop algorithm.
    """),
    ...
    **Where neural nets win:**
    - **Unstructured signals** — images, audio, text, raw sensor streams — where
      features must be *learned*, not engineered (Phases 3–4). Here NNs dominate
      decisively.
    - **Huge datasets with complex interactions** and a need for representation transfer
      (pretraining/fine-tuning, Notebook 22).

    **Where they usually lose (the honest senior take):**
    - **Plain tabular data** — gradient boosting (Notebook 08) typically matches or beats
      a neural net with far less tuning, less data, no scaling, and better
      interpretability (Notebook 13). Reaching for deep learning on a 50-column CSV is a
    ...
    **Constraints:** data modality and volume, latency/compute budget, interpretability
    and regulatory needs (Notebook 13), and team expertise.
    ...
    **KPIs:** accuracy vs a strong GBM baseline, training/inference cost, time-to-ship,
    ...
    - **Initialization & scaling.** Use Xavier/He init and **standardize inputs** —
      both directly affect whether the net trains at all (§7, Notebook 03).
    ...
    - **Overfitting controls** are first-class in production NNs: dropout, weight decay
      (L2, Notebook 04), early stopping, data augmentation, more data.
    - **Reproducibility.** Seed everything; non-convexity (Notebook 03) means runs
      differ — pin seeds and checkpoint.
    - **Interpretability.** NNs are opaque; use SHAP / gradient attributions
      (Notebook 13) and monitor behavior, especially in regulated settings.
    ...
    **Activation functions:**
    ...
    | Tanh | Zero-centered | Saturates | Sometimes hidden (RNNs, Ntbk 17) |
    ...
    **Width vs depth:** wider layers add capacity linearly; **depth** composes features
    ...
    problems — the rationale for "deep" learning (Notebooks 16, 19).
    ...
    **Senior (interview + production design)**
    7. *Whiteboard:* derive the gradients for the 2-layer net (§4.6) from the chain
       rule, then explain how this generalizes to arbitrary depth (preview of Ntbk 15).
    ...
    ---
    ### Summary
    A neural network is **logistic regression stacked in layers** (Notebook 05) with a
    **nonlinearity** between them — and that nonlinearity is everything: it prevents the
    ...
    Notebook 01), cross-entropy loss (Notebook 02), and hand-derived gradients trained
    by gradient descent (Notebook 03).

    **Next:** `15 · Backpropagation` — we generalize the hand-derived 2-layer gradients
...
build("phase3_deep_learning/14_neural_networks_from_scratch.ipynb", cells)
